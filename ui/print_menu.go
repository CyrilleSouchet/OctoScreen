...
func (m *printMenuPanel) initialize() {
	defer m.Initialize()
	m.Grid().Attach(MustButtonImageStyle("Température", "heat-up.svg", "color4", m.showTemperature), 1, 0, 1, 1)
	m.Grid().Attach(MustButtonImageStyle("Ventilateur", "fan.svg", "color2", m.showFan), 2, 0, 1, 1)
	m.Grid().Attach(MustButtonImageStyle("Réseau", "network.svg", "color1", m.showNetwork), 3, 0, 1, 1)
	m.Grid().Attach(MustButtonImageStyle("Système", "info.svg", "color3", m.showSystem), 4, 0, 1, 1)
